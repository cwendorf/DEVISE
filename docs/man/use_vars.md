...
- **`...`**: One or more expressions: A formula (e.g., y ~ x1 + x2) for use with modeling functions. One or more bare variable names (e.g., x1, x2).

### Value

If a single variable is provided: returns a vector. If multiple variables are provided: returns a data frame. If a formula is provided: returns a data frame with the selected variables, and the original formula is preserved as an attribute for use by functions like lm(), glm(), or t.test().

---
title: "Bootstrapped Intervals"
output:
  github_document:
    preserve_yaml: FALSE
vignette: >
  %\VignetteIndexEntry{Bootstrapped Intervals}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include=FALSE}
knitr::opts_chunk$set(comment = NA)
devtools::source_gist("8e6e5dc401e3fc1042ef7a030f9d19c7", filename = "revised_toc.R")
```

## Bootstrapped Intervals

This vignette demonstrates how to use the `confintr` package for parametric and bootstrap confidence intervals. Confintr supports both distribution-based confidence intervals for normally-distributed data and bootstrap methods that provide distribution-free intervals without relying on parametric assumptions.

```{r toc, echo=FALSE}
knitr::current_input() -> thisfile
revised_toc(thisfile, base_level = 3, toc_depth = 4)
```

---

### Load the Packages

```{r, include=FALSE}
if (!require(remotes)) install.packages("remotes")
if (!require(DEVISE)) {remotes::install_github("cwendorf/DEVISE")}
library(DEVISE)
if (!require(confintr)) {remotes::install_github("mayer79/confintr")}
library(confintr)
```

### Case 1: Parametric Confidence Intervals

#### Input the Data

Create a dataset for confidence interval analysis using confintr.

```{r}
gl(3, 10, labels = c("Level1", "Level2", "Level3")) -> Factor
c(6, 8, 6, 8, 10, 8, 10, 9, 8, 7, 7, 13, 11, 10, 13, 8, 11, 14, 12, 11, 9, 16, 11, 12, 15, 13, 9, 14, 11, 10) -> Outcome

data.frame(Factor, Outcome) -> df
```

#### Compute Confidence Intervals

Use confintr's ci_mean function with the default parametric method for each condition.

```{r}
ci_mean(df$Outcome[df$Factor == "Level1"]) |> extract_intervals() -> Level1_Param
ci_mean(df$Outcome[df$Factor == "Level2"]) |> extract_intervals() -> Level2_Param
ci_mean(df$Outcome[df$Factor == "Level3"]) |> extract_intervals() -> Level3_Param

rbind(Level1_Param, Level2_Param, Level3_Param) -> ParamConditions
c("Level1", "Level2", "Level3") -> rownames(ParamConditions)
```

#### Display Confidence Intervals

Format and visualize the parametric confidence intervals.

```{r}
ParamConditions |> style_matrix(title = "Table 1: Parametric Condition Means and Confidence Intervals", style = "apa")
ParamConditions |> plot_conditions(title = "Figure 1: Parametric Condition Estimates", values = TRUE)
```

#### Compute Confidence Interval for Comparison

Use confintr's ci_mean_diff for the parametric method to compare conditions.

```{r}
ci_mean_diff(df$Outcome[df$Factor == "Level2"], 
             df$Outcome[df$Factor == "Level1"]) |> extract_intervals() -> Difference_Param

rbind(Level1_Param, Level2_Param, Difference_Param) -> ParamComparison
c("Level1", "Level2", "Difference") -> rownames(ParamComparison)
```

#### Display Confidence Interval for Comparison

Present the parametric comparison results in tables and plots.

```{r}
ParamComparison |> style_matrix(title = "Table 2: Parametric Means, Confidence Intervals, and Comparison", style = "apa")
ParamComparison |> plot_comparison(title = "Figure 2: Parametric Comparison Results", values = TRUE)
```

### Case 2: Bootstrap Confidence Intervals

#### Compute Confidence Intervals

Use confintr's ci_mean function with bootstrap methods for each condition.

```{r}
ci_mean(df$Outcome[df$Factor == "Level1"], type = "bootstrap", R = 10000) |> extract_intervals() -> Level1_Boot
ci_mean(df$Outcome[df$Factor == "Level2"], type = "bootstrap", R = 10000) |> extract_intervals() -> Level2_Boot
ci_mean(df$Outcome[df$Factor == "Level3"], type = "bootstrap", R = 10000) |> extract_intervals() -> Level3_Boot

rbind(Level1_Boot, Level2_Boot, Level3_Boot) -> BootConditions
c("Level1", "Level2", "Level3") -> rownames(BootConditions)
```

#### Display Confidence Intervals

Format and visualize the bootstrap confidence intervals.

```{r}
BootConditions |> style_matrix(title = "Table 3: Bootstrap Condition Means and Confidence Intervals", style = "apa")
BootConditions |> plot_conditions(title = "Figure 3: Bootstrap Condition Estimates", values = TRUE)
```

#### Compute Confidence Intervals for Comparison

Use confintr's ci_mean_diff with bootstrap method to compare conditions.

```{r}
ci_mean_diff(df$Outcome[df$Factor == "Level2"], 
             df$Outcome[df$Factor == "Level1"], 
             type = "bootstrap", 
             R = 10000) |> extract_intervals() -> Difference_Boot

rbind(Level1_Boot, Level2_Boot, Difference_Boot) -> BootComparison
c("Level1", "Level2", "Difference") -> rownames(BootComparison)
```

#### Display Confidence Intervals for Comparison

Present the bootstrap comparison results in tables and plots.

```{r}
BootComparison |> style_matrix(title = "Table 4: Bootstrap Means, Confidence Intervals, and Comparison", style = "apa")
BootComparison |> plot_comparison(title = "Figure 4: Bootstrap Comparison Results", values = TRUE)
```
